...
- **Lessons are on the Whiteboard, not Zoom.** The app carries its own video call, so the About page no longer says "live on Zoom". The **free assessment** is still a 20-minute Zoom call (the owner's process).
...
- **Free assessment process (as the owner runs it)**: the parent sends the form or a WhatsApp message; Jeremy replies on WhatsApp within 24 hours to fix a 20-minute Zoom slot; the parent sends a photo of the child's latest marked paper to +65 8991 3181 before the call; a written summary of the gaps follows. The contact page steps, the homepage, contact and landing-page form confirmation messages and the chatbot all use this wording. There is **no trial lesson** — say so wherever "free trial" is asked
- **Enquiry forms**: required fields are parent's name, phone/WhatsApp, level and subject (homepage: one subject select; contact: at least one checkbox; `/contact/?subject=english|maths|preib|gp|econ` ticks it). Email is optional. The child's name, the message and "How did you find us?" (on both forms) sit in a `<details>` "More details (optional)". Field names are unchanged for the inbox (`parentName`, `phone`, `email`, `studentLevel`, `subject`). Every lead form carries `data-lead-form`, posts `new FormData(form)`, and on success shows a `role="status"` box with a pre-filled WhatsApp button asking for the marked paper; the submit stays disabled after a send. The four landing pages have their own four-field form (`#lp-form`, directly after the hero, subject fixed); their Book buttons scroll to it and "View Full Programme" became an "Ask on WhatsApp" button. The global sticky CTA is hidden on /contact/. **No reassurance lines under buttons** (guarantee, reply time): the owner declined them on 23 September 2026
