...
│   ├── Footer.astro          # Site footer with links and contact info (column headings are h2 styled as micro-labels: h4 broke heading order on 25 pages)
│   ├── PdfPreviewModal.astro # Full-screen PDF preview overlay for resource samples
│   ├── GuideForm.astro       # Inline free-guide request card (subject select + email → PDF download); see "Subject guides"
...
│   ├── whiteboard.astro      # The A-Worthy Whiteboard: how online lessons work (features, why it works, per-subject use, set-up, FAQ); see "The A-Worthy Whiteboard page"
...
│   │   ├── choosing-jc-subject-combinations.astro # JC 1 subject choice: rules, 70-point UAS, NUS/NTU/SUTD prerequisites (MOE/university-sourced, checked 23 Sep 2026)
│   │   ├── ib-maths-aa-vs-ai.astro # Pre-IB → IB Diploma maths: AA vs AI, SL vs HL, the 2027 syllabus change, NUS/NTU/SMU/SUTD requirements (IB/university-sourced, checked 25 Sep 2026; recheck the university lists each year)
...
│   ├── privacy-policy.astro  # PDPA-compliant privacy policy (linked from the footer)
...
public/
├── images/
│   ├── logo-grade-a.webp      # Site logo (the full grade-a sticker, caption included) — on-page use (Header, Footer, chatbot, offline page). See "Brand Mascots"
│   ├── logo-grade-a.png       # Same mark, PNG — schema.org `logo` field only (Google's documented format)
...
├── docs/guides/              # The five free subject guides the GuideForm delivers (see "Subject guides")
...
- Meta tags (Open Graph with `ogImage` prop, Twitter Card, canonical URL, an RSS alternate link to `/blog/feed.xml`). Blog posts pass `published` / `modified` (the same ISO dates as their BlogPosting JSON-LD), which turns `og:type` into `article` with `article:published_time` / `modified_time`; every other page stays `website`. A new post must pass both
- `<meta name="theme-color">` follows the site theme: the head theme script sets it (#FAF6EE light, #171614 dark) and the Header theme button updates it, so the phone's address bar no longer stays cream above a dark page
...
- Back-to-top button
- Global sticky CTA bar (hidden on homepage and /contact/; shows after half a screen on every visit; a single 60px row of CTA + WhatsApp button + dismiss on phones/tablets, text label only on desktop)
...
- FAQ chatbot widget (pre-defined Q&A). A non-modal dialog: opening it moves focus to the first question, the × or Escape closes it and hands focus back to the round button, which carries `aria-expanded`. Its script wires up at once, not on idle (an idle wait of up to 5s left the button dead to an early click). It is a processed `<script>` (not `is:inline`), so Astro emits it as one hashed `/_astro` file that is cached across pages instead of 7.7 KB inline on every page; the other BaseLayout scripts stay inline because they must run before first paint or in the head
- **Landmarks**: every sitewide widget sits in a labelled landmark so axe's `region` rule passes on all pages: the phone subject selector is `<nav aria-label="Subjects">`, and the exam countdown, the global sticky bar and the WhatsApp float are `<aside>` with an `aria-label`. A new floating or fixed widget needs the same
- Form field messages `window.aworthyFieldErrors(form, [[control, message], ...])` (see "Enquiry forms" under Key Stats)
- **No email pop-up and no privacy notice** (both removed 23 Sep 2026 at the owner's request: the notice covered the booking button on every first phone visit and switched off the sticky bar; the pop-up covered the screen after 45s and delivered a 2 KB title page). Don't reintroduce either. Guides are offered inline by `GuideForm`; a consent prompt is needed only if Clarity is switched on
- Analytics head script: the Plausible queue stub, the delegated click listener, the session attribution record and WhatsApp reference codes (see "Analytics" under Common Pitfalls)
...
| `--z-modal` | 2000 | PDF preview, Study Hub viewer, resource gate, notes lightboxes |
| `--z-toast` | 2100 | Toasts |
...
- **Fixed bottom chrome**: every bar that can occupy the bottom edge — the global sticky CTA (BaseLayout) and the homepage's own `.sticky-cta` — reports its visible height through `window.aworthyBottomBar(name, px)` (defined in BaseLayout's head script; `0` when hidden or dismissed). The registry publishes the **tallest** as `--sticky-bar-h` on `<html>`, and the WhatsApp float, chatbot toggle and back-to-top lift by `translate: 0 calc(var(--sticky-bar-h, 0px) * -1)` (with a `translate` transition), so they ride above whichever bar is showing. They used to add the variable to `bottom`, which re-laid out the page on every change and counted as layout shift. Never write `--sticky-bar-h` directly: each bar used to, and a bar that didn't report (the old cookie notice) left the floats sitting on its buttons. **Any new fixed bottom bar must report through the registry, and any new floating widget must lift by the variable through `translate`, not `bottom`.** The registry also toggles `html.has-bottom-bar` while any bar shows (written only on change). **One float on phones**: at ≤640px the chatbot toggle and back-to-top are hidden, and the subject pill fades out while `has-bottom-bar`; at ≤768px the WhatsApp float fades out while `has-bottom-bar`, because both sticky bars carry their own 44px WhatsApp button (`.sticky-cta-wa`) there. Fade with opacity/visibility/pointer-events, never `display`. Desktop back-to-top sits left of the WhatsApp float (`right: 2rem + 68px`).
...
- Logo — the full grade-a sticker (caption and die-cut border included), no frame: `.logo-img` in the Header is `calc(var(--header-height) - 12px)` tall (60px, 44px in the 56px landscape/PWA header, 84px at 2560), `.footer-logo-img` 80px. Header and Footer pair it with the wordmark "A-Worthy" (800) over the micro-label "Education". The old 44px `.logo-tile` square was removed with the caption-free logo: a portrait sticker inside it shrank to 32px.
...
- One host: the www → apex redirect is a **Vercel domain setting** (Project → Settings → Domains → www.a-worthy.com → Redirect to a-worthy.com, 308), not vercel.json. A `has: host` redirect in vercel.json was tried in September 2026 and never fired in production, so it was removed. Until the domain setting is made, www still answers with 200 (canonical tags point at the apex)
...
- **Navigation** — no client-side router. Every link is a normal page load, so each page's scripts run exactly once. `global.css` ends with `@view-transition { navigation: auto; }` (inside `prefers-reduced-motion: no-preference`) for a native cross-fade in Chrome 126+ / Safari 18.2+. Set only its duration: **never give `::view-transition-old/new(root)` their own keyframes or easing.** BaseLayout had separate ease-out / ease-in fades until 25 September 2026, which left each half at ~31% at the midpoint, so the whole page, header included, blinked on every click; the browser's own cross-fade adds the halves up to full strength. `astro.config.mjs` sets `prefetch: { prefetchAll: true, defaultStrategy: 'hover' }`. Phones have no hover, so the `/contact/` "Get a Free Report" links in the header, the phone menu and the global sticky bar carry `data-astro-prefetch="viewport"`: the contact page loads as soon as one is on screen. Desktop hover prefetches are reused by the navigation through the service worker (tested: one request, served from cache).
- **Service worker** (`public/sw.js`, `VERSION = 'aworthy-v10'`; bumped from v9 on 24 September 2026 because `/offline.html`'s wording changed, and the offline page is precached):
  ...
  Bump `VERSION` whenever the worker's behaviour changes or a precached file (`/offline.html`, the logo, the favicon) changes: activation deletes every other cache, which is how stale data gets purged from returning visitors. Navigation preload is on.
...
12. **Storage keys** — localStorage: `theme` (dark/light), `resource-lead`; sessionStorage: `aw-attr` (first-touch attribution) and `aw-hero-seen` (skip the homepage hero cascade). The old `email-popup-dismissed` and `privacy-accepted` keys are no longer used. **Guard every read and write with `try/catch`** — storage throws (not just returns null) under Safari's "Block All Cookies" and in some in-app browsers, and an unguarded call kills the rest of its script
...
1. Hero with geometric SVG decoration and the letter-by-letter "You are A Worthy" display (Nunito 800; `PenAnimation.astro` is imported but not currently rendered), the `cheer` sticker floating in the right margin (≥1280px only), audience selector (parent/student), the intake line as a `.speech-bubble` callout + `/pricing/` link. The entrance cascade finishes within 0.8s of first paint, the hero buttons never animate, and an inline script before the hero skips the cascade on later homepage views in the same session (`sessionStorage['aw-hero-seen']` → `html.hero-seen`). It used to hold the Book button back for 2.85s. There is **no scroll cue** — the old "Scroll" arrow printed over the audience paragraph and was removed along with its styles
...
## The A-Worthy Whiteboard page (`/whiteboard/`)

Added 24 September 2026 at the owner's request: a page on the online classroom and why it works for exam subjects. It is linked from the homepage Whiteboard band ("How lessons work on the Whiteboard →"), the footer's About column ("Our online classroom"), the About page's online-lessons card and the chatbot's `online` answer.

- **Source of truth for features**: the Whiteboard is the owner's own app at `whiteboard.a-worthy.com`. The page names only features the app's interface shows or the site already states: joining with video, audio only or the board alone; a call that docks beside the board or floats; one shared real-time page with stylus support; documents (past papers) on the board; "Bring everyone to my view"; named pages and board templates; export of every page to one PDF; Documents, Homework and Recordings drawers; link or QR invites with the tutor admitting each student and no student account; runs in a browser tab. Don't add a feature until the app has it.
- **Lessons are on the Whiteboard, not Zoom.** The app carries its own video call, so the About page no longer says "live on Zoom". Nothing on the site uses Zoom any more: the free assessment is a written report, not a call (see Key Stats).
- The page has no FAQPage JSON-LD; if one is added, it must mirror the visible FAQ word for word.

## Subject guides (`GuideForm`)

`src/components/GuideForm.astro` is the inline card that replaced the email pop-up (23 September 2026). The reader picks a guide (preset by the `subject` prop, still changeable), enters an email, and gets a download link; the request posts to Formspree `xreoozkk` with `_subject` "Guide request: <Subject>". `Lead: Guide {subject}` fires only when Formspree answers OK, but the download link shows either way. It carries a one-line PDPA purpose notice with a privacy-policy link; keep it. Placements: a `#free-guide` section before the final CTA on the five subject pages, near the top of /resources/ (no preset), and after the AuthorBio on every blog post (preset by the post's subject; parents' posts have none).

The five PDFs live in `public/docs/guides/` (`o-level-english-guide.pdf`, `o-level-mathematics-guide.pdf`, `pre-ib-mathematics-guide.pdf`, `h1-general-paper-guide.pdf`, `h2-economics-guide.pdf`), 5–6 A4 pages each, built only from frameworks the site already publishes. The .docx sources and the build script live in the session scratchpad, not the repo. `/docs/*` is cached for 30 days, so give a materially changed guide a new filename and update `GuideForm`'s map. The Pre-IB guide embeds DejaVu Sans for √ and ∫ (Nunito has no maths symbols); every other guide is Nunito only.

## Copy voice (the 24 September 2026 rewrite)

Every page, the chatbot and the blog were rewritten in one pass for a genuine, down-to-earth voice that is firm about what we deliver. The owner's words: "Sound genuine, but be firm in our competence and ability to deliver" (they changed "I think we can help" to "we can help"). New copy follows the same rules:

- **Who speaks**: the centre is "we"; Jeremy is "I" only in his first-person story on /about/ and in sign-offs. Speak to "you" and "your child" (student-facing bits speak to the student).
- **How**: plain words and short sentences, contractions (escape them in JS strings), sentence-case headings and buttons (proper nouns and the fixed label **Get a Free Report** keep their capitals), British spelling, at most one em dash per paragraph, no exclamation marks outside quotes.
- **Never**: hype or superlatives ("best", "master", "transform", "unlock", "holistic", "proprietary", "comprehensive"), fake urgency, rhetorical-question openers, sentences that would fit any business, and hedging about our own ability ("I think", "we believe", "hopefully", "may be able to", "aims to"). Firm statements are backed by the method, a specific or a proof point, never by a promised grade.
- **Retention facts, one wording everywhere**: one 90-minute live lesson a week with the same tutor, in a group of at most 6; every lesson recorded, and a missed lesson's recording, worksheet and homework reach the student within 24 hours (no make-up classes); written, marker-style feedback on work every week; a monthly progress report by email (strengths, what to work on, next steps); WhatsApp questions between lessons; our own notes, practice papers and model answers in the fee. Pricing, contact, programmes, the subject pages' level cards and FAQs, the landing pages' closing lines, the homepage parent toggle and the chatbot all use these.
- **Only true services**: claims about how lessons run that could not be confirmed were removed and are listed in CONTENT-TODO.md §8. Don't add a service or promise (parent calls, make-ups, reply times) until the owner confirms it.
- **Shared wording**: every subject page's FAQ heading is "<Subject> tuition: parents' questions", the "Explore more" cards are identical on all five subject pages, the programme is "One-to-one coaching", the WhatsApp button is "WhatsApp us", and "free assessment" / "free diagnostic assessment" are the only names for the assessment. Reading cards on subject pages and the RSS feed use each post's exact H1.
- **Proof**: only the figures in Key Stats below. The O-Level English 90% is always "In 2025, 90% of our O-Level English students scored A1–B3" (or the short label "O-Level English A1–B3").
...
- **Free assessment process (as the owner runs it, changed 24 September 2026)**: the free assessment is a **written feedback report on a test or exam the child did recently**, so the family finds out what went wrong. There is **no call and no Zoom** (the old 20-minute Zoom call was dropped at the owner's request). The parent sends the form or a WhatsApp message; Jeremy replies on WhatsApp within 24 hours; the parent sends a photo of the child's latest test or exam paper to +65 8991 3181; Jeremy goes through it and sends a written report on where the marks were lost, why, what to work on first and the class he recommends. **Turnaround (owner-confirmed, 24 September 2026): the report arrives within 48 hours of the paper; if it's urgent, the parent can call Jeremy on +65 8991 3181** (the same number as WhatsApp; `tel:+6589913181`). The 48 hours and the urgent-call line appear in the contact hero, steps and FAQ, every form confirmation, the chatbot `enrol` / `trial` answers, the homepage contact subtitle, the landing-page form leads, the pricing and Whiteboard CTAs, the final CTA on /programmes/ and the five subject pages, and the closing CTA of every blog post (the urgent-call line is not on the blog). Wording: "a written (feedback) report on your child's latest test or exam"; never "20-minute", "call", "slot" or "Zoom". The contact page steps and FAQ, the homepage, contact and landing-page form confirmation messages (and their pre-filled WhatsApp text "Here is the latest test or exam paper."), every CTA paragraph, the sticky bar, the chatbot `enrol` / `trial` answers and the footer line of the five guide PDFs all use this. There is **no trial lesson** — say so wherever "free trial" is asked
- **Enquiry forms**: required fields are parent's name, phone/WhatsApp, level and subject (homepage: one subject select; contact: at least one checkbox; `/contact/?subject=english|maths|preib|gp|econ` ticks it). Email is optional. The child's name, the message and "How did you find us?" (on both forms) sit in a `<details>` "More details (optional)". Field names are unchanged for the inbox (`parentName`, `phone`, `email`, `studentLevel`, `subject`). Every lead form carries `data-lead-form`, posts `new FormData(form)`, and on success shows a `role="status"` box with a pre-filled WhatsApp button asking for the latest test or exam paper; the submit stays disabled after a send. The four landing pages have their own four-field form (`#lp-form`, directly after the hero, subject fixed); their Book buttons scroll to it and "View Full Programme" became an "Ask on WhatsApp" button. The global sticky CTA is hidden on /contact/. **After a failed send** every enquiry form (homepage, contact, the four landing pages) calls `window.aworthyFieldErrors` (BaseLayout head): each message sits directly under its field (inside the fieldset for the contact subject boxes) linked by `aria-describedby`, goes as soon as the field changes, the first missing field gets focus and is centred if the fixed header or sticky bar covers it, and the box under the button says only "Please check the N fields above." Before this the landing-page form focused its first field behind the header, and /contact/ left focus on the button with the field 637px above the screen. Form controls carry `scroll-margin-top: calc(var(--nav-offset) + 2.5rem)` (plus 48px inside `.prog-section`) and a bottom margin for the sticky bar, in global.css; the helper reads them. **No reassurance lines under buttons** (guarantee, reply time): the owner declined them on 23 September 2026
- **Report buttons**: /about/, /results/, /testimonials/, /success-stories/ and /programmes/ have a Get a Free Report button in the hero (first phone screen); each success story ends with one linking to `/contact/?subject=<key>`
- **Primary CTA (sitewide)**: "Get a Free Report" — used on every CTA button (header, sticky bars, heroes, final CTAs, chatbot links), both contact-form submit buttons (and their JS resets), and all subject pages. It replaced "Book Free Assessment" on 24 September 2026 at the owner's request, because the assessment is a written report and there is no time slot to book. Running copy keeps the offer's name, "free assessment", with the verb "get" ("Get a free assessment", "Get a free diagnostic assessment"), never "book"; pre-filled WhatsApp text reads "I'd like a free assessment for my child"
- **Students taught**: 500+
...
- **Fees (per level)**: O-Level English, O-Level Maths, Pre-IB Maths SGD 280/month (Sec 1–2, Year 3) or SGD 320 (Sec 3–4, Year 4); H1 GP and H2 Economics SGD 360 (JC 1) or SGD 400 (JC 2); one-to-one SGD 120/session (O-Level, IB) or SGD 140 (JC). Discounts: 10% sibling, 5% for two or more subjects. Shown per level on /pricing/ (table and cards), /programmes/, the pricing FAQ + schema and the chatbot
- **Payment terms (owner-confirmed, September 2026)**: one fixed monthly fee paid in advance by the 1st (PayNow preferred, bank transfer, or card via HitPay); a fifth-week lesson costs nothing extra; lessons continue through the school holidays and on public holidays (owner's decisions, 23 September 2026); missed lessons get the recording, no make-ups; Sec 4 and JC 2 groups end after the national examinations, with no fee for the months after the last paper; one month's notice to stop. **Hold**: once a calendar year a family may hold the child's place for up to four consecutive weeks (travel or any reason), with two weeks' written notice (WhatsApp or email); no fee for the held weeks, place kept — pricing FAQ `#pricing-faq-hold` (+ schema), contact FAQ and chatbot `hold` answer. Used in the pricing FAQ "When and how do I pay?" (`#pricing-faq-pay`, opened by hash), its schema, the contact FAQ and the chatbot `payment` answer — change them together
...
- **Situational writing method** (owner's reference documents, 23 September 2026: the *Situational Writing Blueprint*, 2nd ed., and the A-Worthy *Situational Writing Content Bank*): the PAC analysis (Purpose, Audience, Context) with the register fixed in one word; splitting double-barrelled bullets; the five-step routine; the four-move expansion **State, Specify, Show, Sell** (PEEL as the alternative); and a format checklist per text type (CHOPE IT formal letter, SHIOK informal letter, STAMP email, TIKAM report, KENA speech, BOLEH proposal, CLICK article), which must never be written into a script. Marks: Task Fulfilment 10, Language 20 (SEAB 1184), 250–350 words; there is no separate format mark. RAWCP, REED and P-A-R are retired: don't reintroduce them. The email checklist STAMP is unrelated to the STAMP CARD editing framework
- **Syllabus mark facts checked against SEAB (September 2026)**: GP 8881 Paper 1 essay 50 marks (Content 30, Language 20); Paper 2 application question 12 marks. H2 Economics 9570 Paper 1: two case studies of 30 marks each, 60 marks in 2 h 30 min; Paper 2: 75 marks in 2 h 30 min
- **JC admission facts** (used in `/blog/choosing-jc-subject-combinations/`, `/jc-subject-combinations/` and `/blog/ib-vs-jc/`; sourced from MOE and the NUS/NTU/SUTD admissions pages): the UAS maximum is **70** (3 H2 at 20 + GP at 10; the fourth content subject and MTL count only if they raise the score; Project Work is pass/fail but a Pass is required); the contrasting subject may be **H1 or H2**, so a combination that already mixes domains meets the rule; H1 MTL is waived with D7 or better in Higher MTL. University subject requirements are the AY2026 lists: recheck them each year. SMU and per-grade point values could not be verified and are not stated
...
**Primary mark**: since 23 September 2026 the sitewide logo is the **grade-a** pose (the big character holding an "A" paper, the small one with a pencil, an open book and sparkles), at the owner's request. It replaced the "heart2" pose, which had replaced the old navy "A|W" SVG wordmark. It is used in the Header, Footer, chatbot toggle/header, offline page, favicon/apple-touch-icon/manifest icons, and the schema.org `logo` field. The same art is also the O-Level English subject sticker (`mascot-grade-a.webp`), so the English pages show the brand mark as their subject art. **The logo keeps the "A-Worthy Education" caption and the die-cut border** (owner's instruction, 23 September 2026 — a caption-free cut was tried first and rejected). The one exception is the browser-tab favicon (16/32/48px), which uses the caption-free icon variant: at those sizes the caption is two or three rows of pixels and shrinks the characters further. If the favicon needs to read better still, the fix is a favicon-only crop of the big character with his A paper (a crop of the full width slices through the book and the small character).

**File locations**:
- `public/images/logo-grade-a.webp` — the on-page mark (Header, Footer, chatbot toggle/header, offline page): the full sticker, 233×320 (3× the 80px footer height). **The file name changed with the pose** because `/images/*` is served `immutable` for a year: overwriting `logo.webp` in place would have left returning visitors on the heart for up to a year. Rename again if the logo ever changes. WebP because this hand-drawn fabric-texture art compresses far worse as PNG. `public/sw.js` precaches it for the offline page — update the path there and bump `VERSION` when it changes.
- `public/images/logo-grade-a.png` — the same full sticker, PNG, 292×400. Used **only** for the schema.org `logo` URL (in BaseLayout and every blog post's own Article JSON-LD) — Google's documented-supported formats for that property are JPG/PNG/GIF, not WebP.
...
- `public/images/mascots/mascot-*-icon.webp` — the original 9 with the caption cropped off and **no** die-cut border. These are the UI-glyph variants: the ticks (`heart-alt-icon` — still the heart; only the logo changed), checkbox marks, and the audience-toggle tick. List bullets use the `-bullet` copies below. (The About sign-off used `avocado-hug-icon` until 24 September 2026; it now carries the full `feeding` sticker.) Dimensions are unchanged from the first pipeline (the width/height attributes in markup still match). They are no longer *required* for dark backgrounds — the die-cut full variants handle that now — use them where a small, caption-less mark is wanted.
- `public/images/mascots/mascot-grade-a-icon.webp` — the grade-a sticker with the caption and die-cut border removed (372×385); the source of the browser-tab favicons only (the logo files use the full sticker).
- `public/favicon.ico` (multi-size), `public/favicon-16.png`, `public/favicon-32.png`, `public/apple-touch-icon.png` (180×180, opaque cream `#FAF6EE` background, the full captioned sticker at 88% of the height — Apple recommends against transparency), `public/icons/icon-{192,512}x192.png` (same opaque-cream treatment, for PWA manifest / Android home screen). The favicons use the caption-free icon; the home-screen icons use the captioned sticker.
...
**Bullets and checked-state UI** (a separate, sitewide-consistent use, distinct from the accent placements above): the mascot icons replace every plain CSS dot/checkmark `::before` marker, every native list bullet and every native checkbox tick. List bullets rotate through five poses; the checked-state UI keeps the single `mascot-heart-alt-icon.webp`:

- **Bullet markers rotate** (owner's request, 25 September 2026: "rotate between different stickers so that it doesn't show the same one each time", for every bulleted list on the site). global.css gives every `<li>` a `--sticker-bullet` custom property that cycles through five poses by `nth-child(5n+…)`: `heart-alt`, `rocket`, `avocado-hug`, `grade-a`, `heart` (`public/images/mascots/mascot-<pose>-bullet.webp`: 80px-tall copies of the `-icon` files, 22 KB for all five instead of 84 KB, because bullets are drawn at 14–20px; regenerate them if an icon changes). Only these five: `avocado-wink`, `icecream`, `rocket-fly`, `sunglasses` and `fishing` have see-through bodies that turn into grey ghosts in dark mode. Plain lists opt in with `class="sticker-list"` (privacy policy, accessibility, the Whiteboard page's `.wbp-needs`); every blog post's `.blog-post-body ul` gets it automatically. The page-scoped bullet rules — `.founder-band__creds` / `.founder-card__creds` (index / about), `.pricing-card__details` (pricing) and `.prog-card__topics` (programmes + all 5 subject pages, each with its **own** scoped copy) — draw `background: var(--sticker-bullet) center / contain no-repeat`, so they rotate too. A new bulleted list should use `.sticker-list` (or read `var(--sticker-bullet)` in its own `::before`), never a plain disc. The Study Hub materials carry their own copy of the rotation in their inline CSS (they can't read global.css): every `ul` except the topic `.chips` in the editing notes, and the step-card lists and the checklist (its old yellow ticks) in the SHARP quick reference, with `print-color-adjust: exact` so the stickers print. Not bullets, so not converted: numbered lists (`ol`), the before/after ✓/✗ notes (`.ba-notes`, the marks carry meaning) and the results timeline dots. The topic lists' text sits 1.3rem in (0.45rem clear of the 0.85rem sticker) because the square `grade-a` pose touched the text at 1rem.
- **Checkboxes** — Contact's 6 "Which subject(s)?" boxes (one per subject; at least one is required), and the L1R5 calculator's 2 bonus-point boxes. Pattern: the native `<input type="checkbox">` stays in the DOM (visually hidden via `position:absolute;opacity:0`, **not** `display:none`, so it stays keyboard/screen-reader accessible) with a sibling `<span class="check-box" aria-hidden="true">` as the visible bordered box; the mascot icon is that span's `::after`, shown via `input:checked ~ .check-box::after { opacity:1 }`. Each page currently repeats this CSS locally (contact.astro and l1r5-calculator.astro each define their own copy under their own class name) — there's no shared checkbox component to import.
...
- **Breadcrumbs**: The BreadcrumbList JSON-LD is built from the same `pathSegments` / `crumbLabel()` as the visible trail (Home › Blog › post on blog posts; `lp` gets no crumb) and is omitted on `/` and on every `noindex` page. It used to name the second crumb with the full `<title>` and always had two levels. Rendered entirely server-side by Astro in BaseLayout (lines ~226–242). There is no client-side JS breadcrumb script — do not add one, as it would duplicate items. The `breadcrumbLabels` map must include every page slug to avoid bad auto-capitalisation. The visible trail shows labels exactly as written: the old `text-transform: capitalize` on the crumbs was removed (it printed "Choosing A Tuition Centre" and "AA Or AI"), so write each label in the case it should appear.
- **LocalBusiness `geo`**: the EducationalOrganization/LocalBusiness JSON-LD includes a `GeoCoordinates` block. Keep its lat/long in sync with the `geo.position`/`ICBM` meta tags in `<head>`. No `streetAddress` — the centre is online-only (lessons on the A-Worthy Whiteboard), so do not add a fabricated physical address.
...
- The Header component handles both desktop and mobile nav with distinct markup sections. The phone menu (`#mobile-nav`) slides with `transform: translateX()` over 0.3s and is `visibility: hidden` when closed (the visibility step waits for the slide out): until 25 September 2026 it only sat at `right: -320px`, so Tab walked through its 11 links off screen, and the 0.65s slide moved the right edge, laying the page out every frame. Escape and a tap on the overlay return focus to `#hamburger`
...
- **Subject page order and length** (September 2026): hero (with a Get a Free Report button and "From SGD 280/360 per month · groups of up to 6") → level cards → one-to-one → **Schedule → FAQ** → syllabus → approach → frameworks → samples → playbook → before/after → (English: oral) → lesson → testimonials → numbers → reading → CTA → related. The section-nav strip lists Schedule and FAQ right after One-to-One. Long sample material sits inside `<details class="prog-more">` (styles in global.css): frameworks past the first four, the sample-page galleries, the Economics Information Failure example, the Maths marker's report, every SHARP playbook matrix and the English oral model answer. It opens without JavaScript and stays in the HTML. This cut the English page from 42,566 to 22,434px on a 390px phone and brought the timetable from ~38,700px to ~3,700px. Put new long material inside a `prog-more` block, and never move the schedule back to the bottom
...
- Formspree endpoints: the enquiry forms (homepage, contact, landing pages) post to `xgoppwye`; the guide form, resource gate and newsletter post to `xreoozkk`. Each form sends a `_subject` line so the inbox shows what arrived. Lead events fire only when Formspree answers OK
- **Analytics stack**: Plausible (`script.tagged-events.js`) is loaded unconditionally, preceded by the official queue stub so calls before it loads are kept. One delegated click listener in BaseLayout sends `WhatsApp Click` (any wa.me link), `Phone Click` (tel:) and `CTA Click` (any /contact/ link or `[data-cta]`) with props `{page, location}`; `location` is the nearest `[data-track]`, else the nearest `section[id]`, else header/footer/nav/aside. Don't add `plausible-event-name` classes for those three events (double counting). Other events: `Chatbot Open`, `Chatbot Question {q}`, `Lead: Homepage Form`, `Lead: Contact Form`, `Lead: Landing Page Form {subject}`, `Lead: Guide {subject}`, `Lead: Resource Gate`; the older `LP CTA Click` / `Blog CTA Click` classes remain. Each goal must also be created in the Plausible dashboard. **Attribution**: a first-touch record for the session (`sessionStorage['aw-attr']`: landing page, external referrer, utm_*, gclid, fbclid) is exposed as `window.aworthyAttribution()` and added as hidden inputs to every `form[data-lead-form]` (`window.aworthyAttachAttribution(form)` for forms added later). **WhatsApp reference codes**: every wa.me link gets ` [ref: CODE]` once (HOME, CONTACT, PRICING, EN, MA, PIB, GP, EC, LP-EN/MA/GP/EC, BLOG, WEB, plus `/utm_source`); links built by script call `window.aworthyWaRef()`. Microsoft Clarity (heatmaps + session recordings) is gated on the `PUBLIC_CLARITY_PROJECT_ID` env var — the script only renders when the var is set, and the privacy-policy page's Clarity entries are conditional on the same var. To enable, set `PUBLIC_CLARITY_PROJECT_ID` in Vercel (and Cloudflare Pages) and redeploy. CSP in `public/_headers` already allows `*.clarity.ms` and `c.bing.com`.
...
- Blog tables: give a table `class="table-stack"` and a `data-label` on every `<td>` (its column heading) and blog.css turns each row into a labelled card below 640px. A three- or four-column table does not fit 390px, and a sideways-scrolling one hides its last column
...
- The site logo (`public/images/logo-grade-a.webp`/`.png`) is a hand-drawn mascot, not the old navy "A|W" SVG — see "Brand Mascots" above before touching Header/Footer logo markup, favicons, or the schema.org `logo` field. A new logo needs a **new file name** (`/images/*` is immutable) and the service-worker precache path + `VERSION` bump
- Never reintroduce `filter: brightness(0) invert(1)` on `.footer-logo-img` — it crushes the full-color mascot into a flat white silhouette. The footer is now a *light* sand band (`--bg-footer` #F3EDE0 with a 2px ink top rule; charcoal in dark mode), so nothing in it needs inverting; the wordmark ("A-Worthy") stays live text (`.footer-logo-text`) beside the sticker
...
- `html` carries `scrollbar-gutter: stable`: overlays set `overflow: hidden` on `<body>` (Study Hub viewer, PDF preview, tablet menu), and without the reserved gutter the page jumped 7px sideways on Windows when one opened
- The pricing and contact FAQs and the About CTA end with a `.reading-nudge` line (global.css) linking the two parent-decision posts, `/blog/does-my-child-need-tuition/` and `/blog/how-to-choose-a-tuition-centre-singapore/`; until September 2026 only the blog index linked them
