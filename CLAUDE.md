...
- `public/images/mascots/mascot-*-icon.webp` — the original 9 with the caption cropped off and **no** die-cut border. These are the UI-glyph variants: the list bullets and ticks (`heart-alt-icon` — still the heart; only the logo changed), list bullets, checkbox marks, and the audience-toggle tick. (The About sign-off used `avocado-hug-icon` until 24 September 2026; it now carries the full `feeding` sticker.) Dimensions are unchanged from the first pipeline (the width/height attributes in markup still match). They are no longer *required* for dark backgrounds — the die-cut full variants handle that now — use them where a small, caption-less mark is wanted.
