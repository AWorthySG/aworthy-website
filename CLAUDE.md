...
- FAQ chatbot widget (pre-defined Q&A). A non-modal dialog: opening it moves focus to the first question, the × or Escape closes it and hands focus back to the round button, which carries `aria-expanded`. Its script wires up at once, not on idle (an idle wait of up to 5s left the button dead to an early click)
