...
│   │   ├── mascot-{heart,heart-alt,rocket,avocado-wink,avocado-hug}.webp      # full (with caption), light/dark bg
│   │   └── mascot-{heart,heart-alt,rocket,avocado-wink,avocado-hug}-icon.webp # no caption — required on dark navy bg
...
The site's logo and decorative brand illustrations are a hand-drawn "eraser character" mascot set (5 poses: holding a heart ×2, a rocket, and an avocado ×2 — winking / hugging), sourced as 400×400 JPGs with a flat white background and processed into transparent assets. Processing scripts live only in session scratchpads (not checked into the repo) — if you need to reprocess or add a new pose from a similar sticker set, redo the pipeline below rather than looking for a script in-repo.
...
- `public/images/mascots/mascot-{heart,heart-alt,rocket,avocado-wink,avocado-hug}.webp` — "full" versions (character + prop + the sticker's own baked-in "A-Worthy Education" caption), for placement on light/dark page backgrounds.
...
**Keep it sparse.** Mascots currently appear sitewide via Header/Footer/chatbot/email-popup (BaseLayout), plus one accent each on the homepage, About (a small icon-only flourish beside the founder's sign-off — not mid-letter, to avoid undercutting that page's sincerity), 404, Testimonials, Results, and Contact. They were deliberately **not** added to Pricing, Programmes, or any of the five subject pages (`o-level-english`, `h1-general-paper`, `h2-economics`, `o-level-mathematics`, `pre-ib-mathematics`) — those are the highest-stakes conversion/credibility pages, and the sitewide header/footer swap already carries the new brand identity onto them without extra clutter. If asked to add more mascot placements, keep this restraint in mind rather than sprinkling them onto every page.
